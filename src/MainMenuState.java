...
//	private ArrayList<Button> buttons;
	...
	public void init(GameContainer gc, StateBasedGame sbg) throws SlickException {
		...
		startMultiOption = menuOptions.getSubImage(0, 49, 355, 98);
		
		InstructionsOption = menuOptions.getSubImage(0, 98, 355, 147);
		
		exitOption = menuOptions.getSubImage(0, 147, 355, 196);
	...
	public void update(GameContainer gc, StateBasedGame sbg, int arg2) throws SlickException {
		...
		if( ( mouseX >= menuX && mouseX <= menuX + startSingleOption.getWidth()) &&
		    ( mouseY >= menuY && mouseY <= menuY + startSingleOption.getHeight()) ){
		    insideSingleGame = true;
		}else if( ( mouseX >= menuX && mouseX <= menuX+ startMultiOption.getWidth()) &&
		          ( mouseY >= menuY+50 && mouseY <= menuY+50 + startMultiOption.getHeight()) ){
		    insideMultiGame = true;
		}else if( ( mouseX >= menuX && mouseX <= menuX+ InstructionsOption.getWidth()) &&
		          ( mouseY >= menuY+100 && mouseY <= menuY+100 + InstructionsOption.getHeight()) ){
		    insideInstructions = true;
		}else if( ( mouseX >= menuX && mouseX <= menuX+ exitOption.getWidth()) &&
		          ( mouseY >= menuY+150 && mouseY <= menuY+150 + exitOption.getHeight()) ){
		    insideExit = true;
		}
